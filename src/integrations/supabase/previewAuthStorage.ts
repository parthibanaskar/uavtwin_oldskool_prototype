...
export function brokeredPreviewStorage() {
  ...
  const dev =
    host.endsWith(".cloudproject-dev.com") || host.endsWith(".gpt-eng.com");
  const EDITOR = dev
  ...
  const editorOrigins =
    ancestor && EDITOR.test(ancestor)
      ? [ancestor]
      : dev
        ? ["https://cloud.dev", "http://localhost:3000"]
        : ["https://cloud.dev"];
  ...
  return {
    ...
    setItem: (key: string, value: string) => {
      ...
      return request("cloud-preview-auth:set", key, value).then(
        () => undefined,
      );
    },
