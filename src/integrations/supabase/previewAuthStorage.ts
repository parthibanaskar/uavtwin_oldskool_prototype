...
// On a cloud preview surface, broker the auth session to the editor over
// postMessage so the project's preview surfaces share one login; else localStorage.
export function brokeredPreviewStorage() {
  if (typeof window === 'undefined') return undefined;
  const host = location.hostname;
  const PREVIEW_ZONES = ['cloudproject.com', 'cloudproject-dev.com', 'cloud.app', 'gpt-eng.com', 'gptengineer.run'];
  ...
  // Post only to the real editor ancestor, validated as a cloud origin, so the
  // session token can never reach an untrusted embedder.
  const dev = host.endsWith('.cloudproject-dev.com') || host.endsWith('.gpt-eng.com');
  const EDITOR = dev
    ? /^https:\/\/([a-z0-9-]+\.)*(cloud\.dev|gptengineer\.app)$|^http:\/\/localhost:3000$/
    : /^https:\/\/([a-z0-9-]+\.)*(cloud\.dev|gptengineer\.app)$/;
  ...
  const editorOrigins = ancestor && EDITOR.test(ancestor)
    ? [ancestor]
    : (dev ? ['https://cloud.dev', 'http://localhost:3000'] : ['https://cloud.dev']);
  const RESULT = 'cloud-preview-auth:result';
  const TIMEOUT = 2000;
  ...
  return {
    getItem: async (key: string) => {
      let res = await request('cloud-preview-auth:get', key);
      if (!res && firstGet) {
        await new Promise((r) => setTimeout(r, RETRY_DELAY));
        res = await request('cloud-preview-auth:get', key);
      }
      firstGet = false;
    ...
    setItem: (key: string, value: string) => {
      ...
      return request('cloud-preview-auth:set', key, value).then(() => undefined);
    },
    removeItem: (key: string) => {
      localStorage.removeItem(key);
      return request('cloud-preview-auth:remove', key).then(() => undefined);
    },
  };
}
