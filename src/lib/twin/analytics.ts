...
export function deviation(key: ParamKey, value: number, nominal: number): number {
...
export function derive(sample: Sample): Derived {
  ...
  if (sample.physics?.throttle_reduction && sample.physics.throttle_reduction < 1.0) {
    nominal.rpm = nominal.rpm * sample.physics.throttle_reduction;
  }

  const deviations = {} as Record<ParamKey, number>;
  for (const key of PARAM_KEYS) deviations[key] = deviation(key, sample.params[key], nominal[key]);

  const spectrum = sample.spectrum || magnitudeSpectrum(sample.vibWave, sample.vibSampleRate);
  const rotHz = Math.max(4, sample.params.rpm / 60);
  const imbalanceEnergy = bandEnergy(spectrum, rotHz * 0.85, rotHz * 1.15);
  const bearingEnergy =
    bandEnergy(spectrum, rotHz * 3.2, rotHz * 3.95) + bandEnergy(spectrum, rotHz * 6.8, rotHz * 7.5);
  const broadbandEnergy = bandEnergy(spectrum, rotHz * 8, sample.vibSampleRate / 2);

  const redundancy: RedundancyRow[] = [
    row("EGT", sample.params.egt, sample.redundant.egt, 18, "\u00b0C"),
    row("Vibration", sample.params.vibration, sample.redundant.vibration, 0.9, "mm/s"),
    row("Oil pressure", sample.params.oilPressure, sample.redundant.oilPressure, 0.35, "bar"),
  ];

  const trustedVibration =
    redundancy[1]!.status === "fail" ? sample.redundant.vibration : sample.params.vibration;

  const gpsErrorMetres = haversine(sample.gps, sample.inertial);

  const health = computeHealth(deviations, redundancy, gpsErrorMetres, bearingEnergy, rotHz, sample.physics);
...
function row(
  ...
  const relative = a === 0 || b === 0 ? Infinity : Math.abs(a - b) / Math.max(Math.abs(a), 1);
  const status: RedundancyRow["status"] =
    relative > 0.6 && delta > tolerance * 2 ? "fail" : delta > tolerance ? "drift" : "ok";
...
function computeHealth(
  ...
  if (physics?.fatigue_crack_m) {
      ...
      const crackDamage = clamp01((physics.fatigue_crack_m - 0.001) / (0.0025 - 0.001));
  ...
  const overall = Math.round(
    SUBSYSTEMS.reduce((min, sub) => Math.min(min, subsystems[sub]), 100) * 0.6 +
      (SUBSYSTEMS.reduce((sum, sub) => sum + subsystems[sub], 0) / SUBSYSTEMS.length) * 0.4,
  );
 ...
 */
export function estimateRul(history: { t: number; value: number }[]): number | null {
  ...
  if (phys.landing_mode) {
    if (dist > 2.0) {
      ...
      push({
        ...
        hotspot: "avionics",
        contributions: [
          contribution("gps", "Distance to Threshold", 100, `${dist.toFixed(2)} km`),
          contribution("rpm", "Altitude", 80, `${phys.altitude_ft?.toFixed(0) ?? "—"} ft`),
        ],
      ...
      push({
        ...
        hotspot: "avionics",
        contributions: [
          contribution("rpm", "Altitude", 100, `${phys.altitude_ft?.toFixed(0) ?? "—"} ft`),
          contribution("gps", "Distance to Runway", 80, `${(dist * 1000).toFixed(0)} m`),
        ],
...
export function evaluateAlerts(d: Derived): AlertCandidate[] {
  ...
  if (d.sample.physics?.fatigue_crack_m && d.sample.physics.fatigue_crack_m > 0.0012) {
      ...
      push({
        ...
        hotspot: "bearing",
        contributions: [
          contribution("spectral", "Crack length (PINN inferred)", Math.min(100, (mm / 2.5) * 100), `${mm.toFixed(3)} mm`),
          contribution("vibration", "Vibration RMS stress", clamp01(d.trustedVibration / 5) * 100, fmt("vibration", d.trustedVibration)),
        ],
  ...
  if (p.alternatorV !== undefined && p.alternatorV < 13.5 && dev.rpm > 0.2) {
      push({
        ...
        title: "Alternator Misfire Sensing: High electrical ripple indicating misfires",
        severity: "warning",
        confidence: 0.88,
        hotspot: "alternator",
        contributions: []
  ...
  for (const [k, v] of Object.entries(dev)) {
    ...
    if (k !== "vibration" && v > 0.75 && !d.sample.physics?.landing_mode) {
      ...
      push({
        ...
        hotspot: "crank",
        contributions: [
          contribution("rpm", "Deviation Magnitude", clamp01(v) * 100, `${(v * 100).toFixed(0)}% divergence from nominal`),
          contribution("vibration", "Rate of change", 85, "Exponential growth/decay signature detected")
        ],
  ...
  const formatTime = (sec: number) => sec > 7200 ? (sec / 3600).toFixed(1) + " hours" : (sec / 60).toFixed(1) + " mins";

  if (phys?.rul_seconds !== undefined && phys.hypo_rul_seconds !== undefined && phys.mission_time_seconds !== undefined && phys.rul_seconds > 0) {
      ...
      const shouldAdvise = (phys.rul_seconds < phys.mission_time_seconds) || d.sample.activeFaults.includes("rulAdvisory");
      ...
      if (shouldAdvise && !phys.crashed) {
        if (phys.hypo_rul_seconds > phys.mission_time_seconds) {
          push({
            ...
            hotspot: "engine",
            contributions: [
                contribution("rpm", "Current RUL", 100, formatTime(phys.rul_seconds)),
                contribution("rpm", "Projected RUL (-15% RPM)", 100, `${formatTime(phys.hypo_rul_seconds)} (+${(gainedMins > 120 ? gainedMins / 60 : gainedMins).toFixed(1)} ${gainedMins > 120 ? 'hours' : 'mins'})`),
            ],
        ...
        if (
            ...
            push({
              ...
              contributions: [
                  contribution("rpm", "Current RUL", 100, formatTime(phys.rul_seconds)),
                  contribution("gps", "Time to Destination", 100, formatTime(phys.mission_time_seconds)),
              ],
  ...
  const vibDev = deviation("vibration", d.trustedVibration, FLIGHT_PROFILES[d.sample.profile].nominal.vibration);
  const bearingRatio = d.bearingEnergy / Math.max(d.imbalanceEnergy, 0.05);

  if (vibDev > 0.45 && bearingRatio > 0.55) {
    const total = vibDev * 46 + clamp01(d.bearingEnergy / 2.6) * 34 + dev.oilTemp * 20;
    
    // Wire RUL/physics into the XAI for Bearing Wear
    const contributions = [
      contribution("vibration", "Vibration RMS", vibDev * 46, `${fmt("vibration", d.trustedVibration)} vs profile nominal`),
      contribution("spectral", "BPFO band energy", clamp01(d.bearingEnergy / 2.6) * 34, `${d.bearingEnergy.toFixed(2)} @ ~${(d.rotHz * 3.57).toFixed(0)} Hz (3.57x shaft)`),
    ];
    if (dev.oilTemp > 0.3) contributions.push(contribution("oilTemp", "Oil temperature", dev.oilTemp * 20, fmt("oilTemp", p.oilTemp)));
    ...
    push({
      ...
      confidence: clamp01(0.42 + vibDev * 0.4 + clamp01(bearingRatio - 0.55) * 0.3),
      hotspot: "bearing",
      contributions,
  ...
  if (d.imbalanceEnergy > 2.4 && bearingRatio < 0.6) {
    push({
      ...
      hotspot: "propeller",
      contributions: [
        contribution("spectral", "1x shaft-order peak", clamp01(d.imbalanceEnergy / 6) * 58, `${d.imbalanceEnergy.toFixed(2)} @ ${d.rotHz.toFixed(0)} Hz`),
        contribution("vibration", "Vibration RMS", vibDev * 30, fmt("vibration", d.trustedVibration)),
        contribution("rpm", "RPM hunting", clamp01(dev.rpm) * 12, fmt("rpm", p.rpm)),
      ],
  ...
  if (dev.oilPressure > 0.4 || dev.oilTemp > 0.6) {
    push({
      ...
      hotspot: "oilSystem",
      contributions: [
        contribution("oilPressure", "Oil pressure", dev.oilPressure * 55, fmt("oilPressure", p.oilPressure)),
        contribution("oilTemp", "Oil temperature", dev.oilTemp * 33, fmt("oilTemp", p.oilTemp)),
        contribution("vibration", "Vibration RMS", vibDev * 12, fmt("vibration", d.trustedVibration)),
      ],
  ...
  if (dev.egt > 0.55 || dev.cht > 0.6) {
    ...
    push({
      ...
      subsystem: "engine",
      title: sensorSuspect ? "EGT channel disagreement — sensor suspect" : "Hot section overtemperature",
      severity: sensorSuspect ? "advisory" : dev.egt > 0.95 ? "critical" : "warning",
      confidence: sensorSuspect ? 0.55 : clamp01(0.5 + dev.egt * 0.45),
      hotspot: "hotSection",
      contributions: [
        contribution("egt", "Exhaust gas temp", dev.egt * 48, fmt("egt", p.egt)),
        contribution("cht", "Cylinder head temp", dev.cht * 32, fmt("cht", p.cht)),
        contribution("redundancy", "Channel A/B agreement", (sensorSuspect ? 1 : 0) * 20, `${d.redundancy[0]!.delta.toFixed(1)} \u00b0C split`),
      ],
  ...
  // ---------------------------------------------------------------------------------------------------------
    ...
    push({
      ...
      subsystem: "engine",
      title: "CRITICAL: Main Bearing Spalling — Non-Repairable — Immediate Divert",
      severity: "critical",
      confidence: 1.0,
      hotspot: "bearing",
      contributions: [
        contribution("vibration", "Spall fragments on race", 100, "Irreversible"),
      ],
  ...
  // ---------------------------------------------------------------------------------------------------------
  if ((phys?.fault_history_count || 0) >= 2 && !phys?.crashed && !phys?.landed && !phys?.landing_mode && !phys?.bearing_permanently_damaged) {
    push({
      ...
      hotspot: "avionics",
      contributions: [
          contribution("vibration", "Historical Faults", 100, `${phys?.fault_history_count} anomalies`),
      ],
  ...
  if (dev.fuelFlow > 0.35) {
    push({
      ...
      hotspot: "fuelSystem",
      contributions: [
        contribution("fuelFlow", "Fuel flow", dev.fuelFlow * 52, fmt("fuelFlow", p.fuelFlow)),
        contribution("rpm", "RPM shortfall", clamp01(dev.rpm) * 26, fmt("rpm", p.rpm)),
        contribution("egt", "EGT (lean shift)", dev.egt * 22, fmt("egt", p.egt)),
      ],
  ...
  if (dev.busVoltage > 0.4) {
    push({
      ...
      hotspot: "electrical",
      contributions: [
        contribution("busVoltage", "Bus voltage", dev.busVoltage * 74, fmt("busVoltage", p.busVoltage)),
        contribution("rpm", "Generator drive RPM", clamp01(dev.rpm) * 26, fmt("rpm", p.rpm)),
      ],
  ...
  if (dev.rpm > 0.5 && dev.fuelFlow < 0.3) {
    push({
      ...
      contributions: [
        ...
        contribution("egt", "EGT compensation", dev.egt * 40, fmt("egt", p.egt)),
      ],
  ...
  if (d.gpsErrorMetres > 45) {
    push({
      ...
      hotspot: "avionics",
      contributions: [
        contribution("gps", "GNSS vs inertial split", clamp01(d.gpsErrorMetres / 260) * 70, `${d.gpsErrorMetres.toFixed(0)} m divergence`),
        contribution("redundancy", "Satellite count anomaly", d.sample.gpsSats > 13 ? 30 : 6, `${d.sample.gpsSats} vehicles tracked`),
      ],
  ...
  if (d.redundancy[1]!.status === "fail") {
    push({
      ...
      hotspot: "bearing",
      contributions: [
        contribution("redundancy", "Channel A/B disagreement", 78, `A ${d.sample.params.vibration.toFixed(2)} vs B ${d.sample.redundant.vibration.toFixed(2)} mm/s`),
        contribution("vibration", "Channel A amplitude", 22, "flatlined near zero"),
      ],
  ...
  if (phys && phys.mission_distance_km !== undefined) {
    if (phys.crashed) {
      ...
      push({
        ...
        hotspot: "engine",
        contributions: [
          contribution("rpm", "Time to Failure", 100, `${phys.rul_seconds.toFixed(0)} seconds`),
        ],
    ...
    if (phys.fatigue_crack_m > 0.0012 && d.sample.activeFaults.length === 0 && !phys.crashed) {
        push({
          ...
          hotspot: "engine",
          contributions: [
              contribution("rpm", "Paris Law Reversal", 100, "Stress reduction applied")
          ],
    ...
    if (!phys.crashed && phys.rul_seconds > 60) {
      ...
      if (t > 20 && t % 20 === 0) {
        const ambients = [
           { title: "Network Link Stable", text: "Encrypted telemetry downlink maintaining 99.9% packet success rate." },
           { title: "Payload Thermal Check", text: "Optics and IR payload temperatures are nominal. Peltier cooling active." },
           { title: "Airspace Deconfliction", text: "No conflicting traffic detected within 50 km radius via ADS-B." },
           { title: "Battery Balance", text: "Avionics backup battery cells balanced within 0.02V delta." },
           { title: "Fuel Transfer Pump", text: "Wing tank to header tank transfer pump cycled successfully." },
           { title: "GNSS Differential Active", text: "Receiving differential corrections from 12 orbital satellites. Navigation uncertainty < 0.5 meters." },
           { title: "Atmospheric Telemetry", text: `Current atmospheric density: ${phys.air_density_kgm3?.toFixed(3)} kg/m³. ISA Deviation: nominal. Clear skies reported.` }
        ];
      ...
      // Only inject if no manual faults are active and no critical issues exist
        if (t > 25 && t % 40 === 0 && d.sample.activeFaults.length === 0 && !phys.crashed && !phys.landed && phys.mission_distance_km > 10.0 && phys.rul_seconds > 3600) {
          const advisories = [
              { title: "Minor Thermal Fluctuation", text: "Exhaust gas temperature showing minor instability (+2%). ECU automatically adjusting mixture. No pilot action required at this time.\n\n⚠️ Prognostic Note: Such minor operational stress anomalies marginally accelerate Paris Law fatigue crack growth, continuously lowering Remaining Useful Life (RUL) by a fraction of a percent." },
              { title: "Transient Vibration Detected", text: "Short burst of mechanical resonance detected on the prop shaft. Likely a wind gust or minor turbulence. Monitoring closely.\n\n⚠️ Prognostic Note: Accumulated micro-vibrations slowly degrade engine shaft integrity, directly contributing to the gradual reduction of system RUL over the mission." },
              { title: "GNSS HDOP Spike", text: "Navigation uncertainty temporarily spiked to 2.1 meters due to satellite geometry. Multipath mitigation active." },
              { title: "CAN Bus Retries", text: "Minor packet collision on the secondary CAN bus. Flight controller successfully requested retry. Zero data loss." }
          ];
...
export function alertFromCandidate(
  ...
  return {
    ...
    rulMinutes,
    contributions: [...candidate.contributions].sort((a, b) => b.value - a.value),
