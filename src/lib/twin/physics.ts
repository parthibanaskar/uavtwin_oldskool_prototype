...
export function getThermodynamics(
  ...
  const eta_th =
    1 - 1 / Math.pow(ENGINE_SPECS.compression_ratio, ENGINE_SPECS.gamma - 1);
...
export function getWoschniHeatTransfer(
  mean_p_kpa: number,
  mean_T_K: number,
  mean_gas_vel: number,
) {
  return (
    3.26 *
    Math.pow(ENGINE_SPECS.bore, -0.2) *
...
export function integrateParisLaw(
  a: number,
  deltaSigma_MPa: number,
  cycles: number,
) {
  // da/dN = C * (Delta K)^m
  ...
  for (let i = 0; i < cycles; i++) {
    const deltaK =
      FATIGUE_SPECS.Y * deltaSigma_MPa * Math.sqrt(Math.PI * current_a);
...
export function estimateRUL(a: number, deltaSigma_MPa: number, rpm: number) {
  ...
  const coeff =
    FATIGUE_SPECS.C *
    Math.pow(
      FATIGUE_SPECS.Y * deltaSigma_MPa * Math.sqrt(Math.PI),
      FATIGUE_SPECS.m,
    );

  const cycles_remaining =
    (Math.pow(FATIGUE_SPECS.a_crit, m2) - Math.pow(a, m2)) / (coeff * m2);
