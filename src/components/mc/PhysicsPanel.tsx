  ...
  return (
    ...
    >
      <PhysicsMetric
        ...
        formatter={(v) =>
          v > 7200
            ? (v / 3600).toFixed(1) + " hours"
            : (v / 60).toFixed(1) + " min"
        }
      />
      <PhysicsMetric
        label="Fatigue Crack Size"
        value={physics.fatigue_crack_m * 1000}
        unit="mm"
      ...
      />
      <PhysicsMetric
        label="Thermodynamic Eff."
        value={physics.eta_th * 100}
        unit="%"
      ...
      {physics.altitude_ft !== undefined && (
        <PhysicsMetric
          label="Altitude (Density)"
          value={physics.altitude_ft}
          unit="ft"
      ...
      {physics.cumulative_damage_pct !== undefined && (
        <div
          className={cn(
            ...
            physics.cumulative_damage_pct > 75
              ? "bg-red-500/20 border-red-500/50"
              : physics.cumulative_damage_pct > 40
        ...
        >
          ...
          <p
            className={cn(
              "font-mono text-lg leading-tight font-semibold mt-1",
              physics.cumulative_damage_pct > 75
                ? "text-red-400"
                : physics.cumulative_damage_pct > 40
                  ? "text-amber-400"
          ...
          >
            {physics.cumulative_damage_pct.toFixed(1)}
      ...
      {physics.bearing_permanently_damaged && (
        <div className="rounded-sm border p-2 bg-red-600/20 border-red-600/60 col-span-2 animate-pulse">
          ...
          <p className="font-mono text-xs text-red-300 mt-1">
            Non-repairable. Elevated vibration for remainder of flight. Divert
            mandatory.
          </p>
        </div>
      )}
      <div
        className={cn(
          "rounded-sm border p-2 col-span-2",
          physics.gpsSpoofed
            ? "bg-red-500/20 border-red-500/50"
            : "bg-muted/20 border-border/70",
        )}
      >
        <div className="flex justify-between">
          ...
          <span
            className={cn(
              "font-mono text-xs",
              physics.gpsSpoofed ? "text-red-500 font-bold" : "text-green-500",
            )}
          >
...
function PhysicsMetric({
  ...
  return (
    ...
    >
      <p className={cn("label-xs truncate", alert && "text-red-400")}>
        {label}
      </p>
