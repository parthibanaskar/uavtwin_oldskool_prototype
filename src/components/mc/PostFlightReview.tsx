...
function Stat({ label, value, alert, sub }: { label: string; value: string; alert?: boolean; sub?: string }) {
  return (
    <div className={cn("rounded border p-3", alert ? "border-red-500/40 bg-red-500/10" : "border-border/50 bg-muted/10")}>
      <p className={cn("text-[0.6rem] uppercase tracking-wider", alert ? "text-red-400" : "text-muted-foreground")}>{label}</p>
      <p className={cn("font-mono text-xl font-bold mt-1 leading-tight", alert ? "text-red-300" : "text-white")}>{value}</p>
      {sub && <p className="text-[0.6rem] text-muted-foreground mt-0.5">{sub}</p>}
    </div>
  );
}
...
export function PostFlightReview() {
  ...
  const majorAlerts = alerts.filter(a => a.severity === "critical" || a.severity === "warning");

  // Outcome
  const outcome = isCrashed
    ? { label: "HULL LOSS — AIRCRAFT CRASHED", color: "text-red-500", borderTop: "border-red-500", icon: XCircle, iconColor: "text-red-500" }
    : isDiverted
    ? { label: "MISSION ABORTED — SAFELY DIVERTED TO FOB", color: "text-amber-400", borderTop: "border-amber-500", icon: AlertTriangle, iconColor: "text-amber-400" }
  ...
  if (isCrashed) {
    recs.push({ label: "Full hull inspection required before any next flight", severity: "critical" });
    recs.push({ label: "Engine shaft fractured — replace engine assembly entirely", severity: "critical" });
  }
  if (bearingDamaged) {
    recs.push({ label: "Main bearing race spalled — full bearing replacement mandatory", severity: "critical" });
    recs.push({ label: "Oil gallery flush required — metal fragment contamination likely", severity: "critical" });
  }
  if (damagePct > 75) recs.push({ label: `Shaft at ${damagePct.toFixed(0)}% structural life — engine overhaul required`, severity: "critical" });
  else if (damagePct > 40) recs.push({ label: `Shaft at ${damagePct.toFixed(0)}% structural life — schedule inspection`, severity: "warning" });

  faultHistory.forEach(f => {
    if (f === "oilStarvation") recs.push({ label: "Oil pump inspection and gallery flush required", severity: "warning" });
    if (f === "bearingWear") recs.push({ label: "Full bearing replacement — mid-flight spalling detected", severity: "critical" });
    if (f === "fuelBlockage") recs.push({ label: "Primary fuel filter replacement required", severity: "warning" });
    if (f === "fuelPumpDegrade") recs.push({ label: "Fuel pump impeller replacement recommended", severity: "warning" });
    if (f === "propImbalance") recs.push({ label: "Propeller blade inspection and static re-balance", severity: "warning" });
    if (f === "egtOvertemp") recs.push({ label: "Turbine blade and combustion chamber inspection", severity: "warning" });
    if (f === "icing") recs.push({ label: "Anti-ice system inspection and pitot heat check", severity: "warning" });
    if (f === "busSag") recs.push({ label: "Alternator output voltage check — ripple anomaly logged", severity: "warning" });
  });
  if (recs.length === 0) recs.push({ label: "No critical anomalies — standard post-flight check sufficient", severity: "ok" });
        ...
        <div className="p-4 sm:p-6 space-y-5 overflow-y-auto custom-scrollbar">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-[0.6rem] uppercase tracking-[0.2em] text-muted-foreground">
                Post-Flight Review • Session {displayed?.sample.missionId ?? "—"}
              </p>
              <h2 className={cn("text-xl font-bold mt-0.5 leading-tight", outcome.color)}>{outcome.label}</h2>
              <p className="text-xs text-muted-foreground mt-1">
                Flight duration: {flightTime} • Anomalies logged: {faultHistory.length} • Fleet Health: {health?.overall ?? 0}%
              </p>
            </div>
          </div>
          ...
          {bearingDamaged && (
            <div className="flex items-center gap-3 rounded border border-red-500/50 bg-red-950/30 px-4 py-3">
              ...
              <div>
                <p className="text-xs font-bold text-red-300">Main Bearing — Permanently Spalled</p>
                <p className="text-[0.65rem] text-red-400/70 mt-0.5">Metal fragments circulated through oil gallery. Full teardown inspection required.</p>
              </div>
            </div>
          )}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {faultHistory.map((f, i) => (
                  <div key={i} className="flex items-center gap-1.5 rounded border border-amber-500/30 bg-amber-500/10 px-2.5 py-1.5">
                    <span className="text-[0.55rem] text-muted-foreground font-mono">#{i + 1}</span>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <p className="text-[0.65rem] uppercase tracking-widest text-muted-foreground mb-2 flex items-center gap-2">
                <AlertTriangle className="size-3.5" /> Active Alerts at Termination
              </p>
              <div className="space-y-1">
                {majorAlerts.length === 0 ? (
                  ...
                  <div key={a.id} className={cn(
                    ...
                    a.severity === "critical" ? "border-red-500/30 bg-red-500/10 text-red-300" : "border-amber-500/20 bg-amber-500/8 text-amber-300"
                  )}>
                    <span className="font-bold uppercase text-[0.6rem] mr-1">{a.severity}</span>{a.title}
                  </div>
                ))}
              </div>
            </div>
            <div>
              <p className="text-[0.65rem] uppercase tracking-widest text-muted-foreground mb-2 flex items-center gap-2">
                <CheckCircle2 className="size-3.5 text-green-400" /> Mitigations Executed
              </p>
              <div className="space-y-1">
                {isDiverted && (
                  <div className="text-xs rounded border border-primary/30 bg-primary/10 text-primary px-2 py-1">
                    <span className="font-bold uppercase text-[0.6rem] mr-1">COMMANDED</span>Emergency divert to FOB
                  </div>
                )}
                {healActions.length === 0 && !isDiverted && (
                  <p className="text-xs text-muted-foreground italic">No interventions required</p>
                )}
                {healActions.slice(0, 3).map(h => (
                  <div key={h.id} className="text-xs rounded border border-green-500/20 bg-green-500/8 text-green-300 px-2 py-1">
                    <span className="font-bold uppercase text-[0.6rem] mr-1">EXECUTED</span>{h.action}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              {uniqueRecs.map((r, i) => (
                <div key={i} className={cn(
                  ...
                  r.severity === "warning" ? "border-amber-500/30 bg-amber-500/5" :
                  "border-green-500/30 bg-green-500/5"
                )}>
                  {r.severity === "critical" ? <XCircle className="size-3.5 text-red-400 mt-0.5 shrink-0" />
                    : r.severity === "warning" ? <AlertTriangle className="size-3.5 text-amber-400 mt-0.5 shrink-0" />
                    : <CheckCircle2 className="size-3.5 text-green-400 mt-0.5 shrink-0" />}
                  <p className={cn("text-xs", r.severity === "critical" ? "text-red-300" : r.severity === "warning" ? "text-amber-300" : "text-green-300")}>{r.label}</p>
                </div>
              ))}
            </div>
          </div>
