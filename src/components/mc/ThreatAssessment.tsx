...
export function ThreatAssessment({ className }: { className?: string }) {
  ...
  return (
    <Panel
      title="Mission Resilience Assessment"
      subtitle="Structural capacity & threat history"
      className={className}
    >
      <div className="flex flex-col gap-3 p-3">
          ...
          {majorResolved.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-1">
              {majorResolved.map((a, i) => (
                <Chip
                  key={i}
                  className="bg-warn/10 text-warn border-warn/20 text-[0.6rem]"
                >
                  {a.title}
                </Chip>
              ))}
        ...
        <div
          className={cn(
            "border border-border/50 rounded-sm p-3",
            critical
              ? "bg-crit/10 border-crit/30"
              : canSustain
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            {critical ? (
              <Anchor className="size-4 text-crit" />
            ) : canSustain ? (
            ...
            <span
              className={cn(
                ...
                critical ? "text-crit" : canSustain ? "text-ok" : "text-warn",
              )}
            >
              {critical
          ...
          <p className="text-[0.7rem] text-muted-foreground leading-relaxed">
            {critical ? (
              <>
                Airframe integrity has dropped below survivable thresholds.
                Structural capacity CANNOT sustain any further anomalies. Revert
                to safe base immediately.
              </>
            ) : canSustain ? (
              <>
                RUL ({rulHours.toFixed(1)} hrs) is sufficient to complete
                mission parameters. System CAN sustain an additional
                catastrophic fault and limp to base.
              </>
            ) : (
              <>
                RUL ({rulHours.toFixed(1)} hrs) is degraded. System may NOT
                sustain another major physical threat. Recommend monitoring for
