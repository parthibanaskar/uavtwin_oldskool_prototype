...
export function AlertFeed({ className }: { className?: string }) {
  const { alerts, setFocusHotspot, clearFault, reduceThrottle, setFuelPath, setNavMode, applyHealAction, resolvedAlerts, setResolvedAlerts, divert, commandSafeLanding } = useMission();
    ...
    >
      <div className="absolute inset-0 overflow-y-auto p-2 pb-6 space-y-3">
        ...
        {[...alerts].reverse().map((a) => {
          ...
          const isProblem = a.severity === "critical" || a.severity === "warning";
          const Icon = isProblem ? AlertTriangle : (a.severity === "nominal" ? ShieldCheck : Info);
            ...
            >
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <Chip tone={a.rulMinutes !== null && a.rulMinutes < 12 ? "crit" : "muted"}>
                    RUL {a.rulMinutes === null ? "stable" : `${a.rulMinutes.toFixed(0)} min`}
                  </Chip>
              ...
              <div className="mt-2 space-y-2 border-t border-border/60 pt-2">
                <p className="text-xs text-muted-foreground whitespace-pre-wrap">{a.narrative}</p>
                    ...
                    <div key={c.key + c.label}>
                      ...
                      <div className="h-1 w-full overflow-hidden rounded-full bg-muted">
                        <div
                          ...
                          style={{ width: `${(Math.abs(c.value) / total) * 100}%` }}
                        />
                      </div>
                    </div>
                  ))}
              ...
              {resolvedAlerts.has(a.id) && a.severity !== 'info' && a.severity !== 'nominal' ? (
                <div className="w-full rounded-sm bg-green-500/10 p-2 text-left border border-green-500/30">
                  ...
                  <div className="text-[0.65rem] text-muted-foreground leading-tight whitespace-pre-wrap">
                    {a.resolutionNarrative || (a.key === "prescriptiveThrottle" 
